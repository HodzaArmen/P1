def preberi_zemljevid(ime_dat):
    with open(ime_dat, 'r') as datoteka:
        zemljevid = [vrstica.strip() for vrstica in datoteka]
    return zemljevid
...
def najblizji(x, y, c, zemljevid):
    ...
    for vrstica in range(len(zemljevid)):
        for stolpec in range(len(zemljevid[vrstica])):
            trenutna_crka = zemljevid[vrstica][stolpec]
            if trenutna_crka == c and (stolpec != x or vrstica != y):
                razd = razdalja((x, y), (stolpec, vrstica))
                if razd < najmanjsa_razdalja or (razd == najmanjsa_razdalja and stolpec < najblizje[0]):
                    najmanjsa_razdalja = razd
                    najblizje = (stolpec, vrstica)
            elif not c and trenutna_crka != '.' and (stolpec != x or vrstica != y):
                razd = razdalja((x, y), (stolpec, vrstica))
                if razd < najmanjsa_razdalja or (razd == najmanjsa_razdalja and stolpec < najblizje[0]):
                    najmanjsa_razdalja = razd
                    najblizje = (stolpec, vrstica)
    return najblizje if najblizje else None
...
def najpogostejsi(x, y, d, zemljevid):
    ...
    for vrstica in range(len(zemljevid)):
        for stolpec in range(len(zemljevid[vrstica])):
            razd = razdalja((x, y), (stolpec, vrstica))
            trenutni_znak = zemljevid[vrstica][stolpec]
            if razd <= d and trenutni_znak != '.':
                if trenutni_znak in mnozica:
                    mnozica[trenutni_znak] += 1
                else:
                    mnozica[trenutni_znak] = 1
    if not mnozica:
        return None
    return min(mnozica, key=lambda x: (-mnozica[x], x))
...
def vsi_najpogostejsi(x, y, d, zemljevid):
    ...
    if not mnozica:
        return set()
    return {x for x,y in mnozica.items() if y == max(mnozica.values())}

def najdi_vse_c(vrstica, c):
    indeksi_c = []
    for indeks, znak in enumerate(vrstica):
        if znak == c:
            indeksi_c.append(indeks)
    return indeksi_c
...
def najblizji(x, y, c, zemljevid, prepovedani):
    ...
    if c == "":
        for vrstica in range(len(zemljevid)):
            for stolpec in range(len(zemljevid[vrstica])):
                trenutna_crka = zemljevid[vrstica][stolpec]
                if trenutna_crka != "." and (stolpec != x or vrstica != y) and (stolpec, vrstica) not in prepovedani:
                    razd = razdalja((x, y), (stolpec, vrstica))
                    if razd < najmanjsa_razdalja or (razd == najmanjsa_razdalja and stolpec < najblizje[0]):
                        najmanjsa_razdalja = razd
                        najblizje = (stolpec, vrstica)
    else:
        for vrstica, vrst in enumerate(zemljevid):
            #vec c-jev v vrstici
            mnozica = najdi_vse_c(vrst, c)
            for stolpec in mnozica:
                if stolpec != -1 and (stolpec != x or vrstica != y) and (stolpec, vrstica) not in prepovedani:
                    razd = razdalja((x, y), (stolpec, vrstica))
                    if razd < najmanjsa_razdalja or (razd == najmanjsa_razdalja and stolpec < najblizje[0]):
                        najmanjsa_razdalja = razd
                        najblizje = (stolpec, vrstica)
...
def angelca(x, y, znamenitosti, zemljevid):
    ...
    for c in znamenitosti:
        nov_par = najblizji(*par, c, zemljevid, mnozica)
        if nov_par is None:
            break
        mnozica.add(nov_par)
        raz += razdalja(par, nov_par)
        par = nov_par
    return raz
...
def johanca(x, y, pot, zemljevid):
    ...
    while index < len(pot):
        ...
        if c.isnumeric():
            st_str = c
            index += 1
            while index < len(pot) and pot[index].isnumeric():
                st_str += pot[index]
                index += 1
            st = int(st_str)
        smer = pot[index]
        ...
        if smer == ">":
            ...
            break
        if 0 <= x < len(zemljevid[0]) and 0 <= y < len(zemljevid) and (y, x) not in mnozica and zemljevid[y][x] != ".":
            mnozica.add((y, x))
            niz += zemljevid[y][x]
    return niz

def najboljsa_cetrt(a, zemljevid):
    najboljsa_koordinata = (0, 0)
    najvec_znamenitosti = 0
    for vrstica in range(len(zemljevid) - a + 1):
        for stolpec in range(len(zemljevid[vrstica]) - a + 1):
            st_znamenitosti = 0
            for i in range(a):
                for j in range(a):
                    if zemljevid[vrstica + i][stolpec + j] != ".":
                        st_znamenitosti += 1
            if st_znamenitosti > najvec_znamenitosti:
                najboljsa_koordinata = (stolpec, vrstica)
                najvec_znamenitosti = st_znamenitosti
            elif st_znamenitosti == najvec_znamenitosti:
                najboljsa_koordinata = min(najboljsa_koordinata, (stolpec, vrstica))
    return najboljsa_koordinata
