def nove_povezave(pot, zemljevid):
    return {(pot[i], pot[i+1]) for i in range(len(pot) - 1) if (pot[i], pot[i+1]) not in zemljevid}

def obiskane_tocke(pot):
    return set(pot)

def popravljena_pot(pot):
    return "".join([pot[i] for i in range(len(pot)) if i==0 or pot[i] != pot[i-1]])
def povezave_z_vescino(pot, zemljevid, vescina):
    return [
        (pot[i], pot[i + 1])
        for i in range(len(pot) - 1)
        if pot[i + 1] in zemljevid.get(pot[i], {}) and vescina in zemljevid[pot[i]][pot[i + 1]]
    ]
